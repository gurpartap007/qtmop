    ...
    ui(new Ui::numeric_keypad)
{
    ...
    qDebug() << "Initializing memory of user input with NULL";
    input_ptr = &input[0];
    connect(this,SIGNAL(clear()),this,SLOT(clear_input()));
    qDebug() << "Connected Clear_input with the clear signal of current class";
}

numeric_keypad::~numeric_keypad()
{
    delete ui;
    qDebug() << "deleted User Interface";
}
...
void numeric_keypad::on_backspace_clicked()
{
    if(&input[0] != input_ptr)
    {
        ...
        *input_ptr = '\0';
        emit value_changed();
        qDebug()    << "Character from left side deleted";
    }
}
